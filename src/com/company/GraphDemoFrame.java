...
public class GraphDemoFrame extends JFrame {
    ...
    private JButton buttonDfsFindWays;
    ...
    public GraphDemoFrame() {
        ...
        buttonCreateGraph.addActionListener(e -> {
            try {
                ...
                Class clz = Class.forName("com.company." + className);
        ...
        buttonDfsFindWays.addActionListener(e -> {
            try {
                if (graph == null) {
                    return;
                }
                showSystemOut(() -> {
                    ...
                    boolean flag = false;
                    if (ways.size() > 1) {
                        for (int i = 1; i < ways.get(0).length()-1; i++){
                            for (int j = 0; j < ways.size(); j++ ){
                                k = ways.get(j).indexOf(ways.get(0).charAt(i));
                                if (k == -1) break;
                                if (j == (ways.size() - 1)){
                                    System.out.print(ways.get(0).charAt(i) + " ");
                                    flag = true;
                                }
                            }
                        }
...
// DO NOT EDIT OR ADD ANY CODE HERE!
    ...
    private void $$$setupUI$$$() {
        ...
        buttonDfsFindWays = new JButton();
        buttonDfsFindWays.setText("dfs (рекурсия)");
        panel5.add(buttonDfsFindWays, new GridConstraints(0, 2, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_HORIZONTAL, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW, GridConstraints.SIZEPOLICY_FIXED, null, null, null, 0, false));
